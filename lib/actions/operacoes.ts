...
export async function duplicarOperacao(id: string) {
  ...
  const copia = await prisma.operacao.create({
    data: {
      ...
      fase: original.fase,
      minutoEntrada: original.minutoEntrada,
      ...
      criterioEntrada: original.criterioEntrada,
      criterioSaida: original.criterioSaida,
...
export async function importarOperacoesPorNome(rows: OperacaoImportRow[]) {
  ...
  for (const linha of rows) {
    ...
    const data = operacaoSchema.parse({
      ...
      criterioEntrada: linha.criterioEntrada,
      criterioSaida: linha.criterioSaida,
      status: linha.status,
